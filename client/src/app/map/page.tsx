...
export default function MapPage() {
  ...
  const [memoryNodes, setMemoryNodes] = useState<MemoryNode[]>(DEMO_NODES);
  ...
  useEffect(() => {
    const fetchMemories = async () => {
      try {
        ...
        if (!response.ok) {
          toast.warning({ title: "地图数据暂不可用", description: "已切换为本地演示点位。" });
          setMemoryNodes(DEMO_NODES);
          return;
        }
        ...
        setMemoryNodes([...DEMO_NODES, ...mapped]);
      } catch {
        toast.warning({ title: "后端连接失败", description: "地图已回退到本地演示数据。" });
        setMemoryNodes(DEMO_NODES);
      }
    };
